 ...
 * Integrates the controller, FSM, and shared context into a single ROS 2 gateway.
 */
NavigatorSystem::NavigatorSystem(std::shared_ptr<PuzzlebotController> controller,
  std::shared_ptr<FiniteStateMachine> fsm, std::shared_ptr<RobotContext> ctx) :
  Node("navigator_system"),
  ctrl_(controller), fsm_(fsm), ctx_(ctx) {

  // Setup ROS 2 communication infrastructure
  setup_communication();

  RCLCPP_INFO(this->get_logger(), "Navigator System: [ACTIVE]. Waiting for perception streams.");
}
...
void NavigatorSystem::setup_communication() {
  ...
  auto wait_for_topic = [this](const std::string& topic_name) {
    RCLCPP_INFO(this->get_logger(), "Connecting to: %s ...", topic_name.c_str());
    while (this->count_publishers(topic_name) == 0) {
      if (!rclcpp::ok()) return;
      rclcpp::sleep_for(std::chrono::milliseconds(500));
    }
    RCLCPP_INFO(this->get_logger(), "Connection established with: %s", topic_name.c_str());
  };
  ...
  sign_sub_ = this->create_subscription<puzzlebot_interfaces::msg::TrafficSignDetection>(
    sign_topic, sensor_qos,
    std::bind(&NavigatorSystem::traffic_sign_callback, this, std::placeholders::_1));

  line_sub_ = this->create_subscription<puzzlebot_interfaces::msg::RoadPerception>(
    line_topic, sensor_qos, 
    std::bind(&NavigatorSystem::line_callback, this, std::placeholders::_1));
 ...
 * 
 * Handles high-frequency steering and specialized landmark events like intersections.
 */
void NavigatorSystem::line_callback(
  ...
  if (msg->crosswalk_detected) {
    ...
    if (ctx_->direction.has_value()) {
      ...
      // Temporal Filtering: Ignore signs from previous intersections (> 3.0s ago)
      if (data_age > 3.0) {
        RCLCPP_DEBUG(this->get_logger(), "Stale direction data (%.2f s). Forcing 'go_ahead'.", data_age);
