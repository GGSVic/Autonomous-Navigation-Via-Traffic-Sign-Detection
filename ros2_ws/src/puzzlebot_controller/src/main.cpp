...
int main(int argc, char** argv) {
  // 1. Initialize ROS 2 communication infrastructure
  ...
  // 4. State Registry & Transition Map
  // Logic: Current State -> { { "Event", "Next State" }, ... }
  
  // -- Global Navigation & Perception States --
  fsm->add_state(std::make_shared<FreeFlow>(), {
    {"tf_red", "CROSS_APPROACH"}, {"tf_yellow", "CROSS_APPROACH"},
    {"give_way", "CROSS_APPROACH"}, {"stop", "CROSS_APPROACH"},
    {"max_speed", "CAUTION_FLOW"}, {"roadwork", "CAUTION_FLOW"},
    {"street_crossing", "MANEUVER_HUB"}
  });

  fsm->add_state(std::make_shared<CrossApproach>(), {
    {"street_crossing", "TRAFFIC_HUB"}, {"tf_green", "FREE_FLOW"}
  });

  fsm->add_state(std::make_shared<CautionFlow>(), {
    {"timeout", "FREE_FLOW"}, {"tf_red", "CROSS_APPROACH"}, 
    {"tf_yellow", "CROSS_APPROACH"}, {"give_way", "CROSS_APPROACH"}, 
    {"stop", "CROSS_APPROACH"}, {"street_crossing", "MANEUVER_HUB"}
  });

  // -- Intersection Handling (Maneuver Hub) --
  fsm->add_state(std::make_shared<ManeuverHub>(), {
    {"go_ahead", "M_STRAIGHT"}, {"turn_right", "M_TURN_RIGHT"}, 
    {"turn_left", "M_TURN_LEFT"}, {"roundabout", "M_U_TURN"}
  });
  ...
  fsm->add_state(std::make_shared<TrafficHub>(), {
    {"yield", "WAIT_YIELD"}, {"stop", "WAIT_STOP"}, 
    {"wait_for_green", "WAIT_LIGHT"}, {"proceed", "MANEUVER_HUB"}
  });
  ...
  RCLCPP_INFO(rclcpp::get_logger("main"), "Puzzlebot Navigator System is now Spinning...");
  ...
  // 8. Graceful Shutdown
  RCLCPP_INFO(rclcpp::get_logger("main"), "Shutting down Puzzlebot Navigation...");
  rclcpp::shutdown();
  return 0;
}