...
FiniteStateMachine::FiniteStateMachine(
    ...
    : Node(fsm_name), robot_ctrl_(robot_ctrl), ctx_(ctx), blocked_(true) {

  // Convert seconds to milliseconds for ROS 2 wall timer
  auto millis =
      std::chrono::milliseconds(static_cast<int>(loop_rate_s * 1000.0));

  timer_ = this->create_wall_timer(
      millis, std::bind(&FiniteStateMachine::control_callback, this));

  RCLCPP_INFO(this->get_logger(), "FSM: [%s] initialized at %.2f Hz.",
              fsm_name.c_str(), 1.0 / loop_rate_s);
}
 ...
 */
void FiniteStateMachine::add_state(std::shared_ptr<State> state,
  ...
  if (states_registry_.find(alias) != states_registry_.end()) {
    RCLCPP_WARN(this->get_logger(),
                "State '%s' already exists. Registration skipped.",
                alias.c_str());
    return;
  }

  // Dependency injection: providing the state with the tools it needs to
  // operate
  ...
  if (current_state_alias_.empty()) {
    ...
    RCLCPP_INFO(this->get_logger(), "FSM Entry Point defined as: [%s]",
                alias.c_str());
  }
}
...
void FiniteStateMachine::start() {
  if (current_state_alias_.empty()) {
    RCLCPP_ERROR(this->get_logger(),
                 "FSM Start Failed: No states registered in the registry.");
    return;
  }
  blocked_ = false;
  ...
  if (state_it != state_transitions_.end()) {
    ...
    if (event_it != transitions.end()) {
      ...
      RCLCPP_INFO(this->get_logger(), "TRANSITION: [%s] --(%s)--> [%s]",
                  current_state_alias_.c_str(), event.c_str(),
                  next_state_alias.c_str());
      ...
      RCLCPP_DEBUG(this->get_logger(),
                   "Event '%s' ignored in state '%s' (No transition defined).",
...
void FiniteStateMachine::control_callback() {
  ...
  if (it != states_registry_.end()) {
    ...
    RCLCPP_ERROR(this->get_logger(),
                 "FSM Runtime Error: Current state '%s' not found in registry!",
                 current_state_alias_.c_str());
  }
}

std::string FiniteStateMachine::get_state() { return current_state_alias_; }